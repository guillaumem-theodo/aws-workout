...
resource "aws_internet_gateway" "igw-203" {
...
resource "aws_route_table" "route-table-203" {
  vpc_id = data.terraform_remote_state.vpc-101.outputs.net-101-vpc-id

  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = aws_internet_gateway.igw-203.id
  }
...
resource "aws_route_table_association" "rt-association-subnet1-203" {
  route_table_id = aws_route_table.route-table-203.id
...
resource "aws_security_group" "sg-203" {
  vpc_id = data.terraform_remote_state.vpc-101.outputs.net-101-vpc-id
...
resource "aws_instance" "public-ec2" {
  ...
  subnet_id = data.terraform_remote_state.subnets-102.outputs.net-102-subnet-1-id
  security_groups = [aws_security_group.sg-203.id]
...
output "cpu-203-rt-id" {
  value = aws_route_table.route-table-203.id
}
