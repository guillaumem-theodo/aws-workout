...
ssh -i ./aws-workout-key-pair.pem -o ConnectTimeout=10 ec2-user@"$ec2_3_public_ip" ping -c 2 "$ec2_2_private_ip"

echo "✅ Test Connection from EC2 in VPC 2 ($ec2_2_private_ip using forwarding agent) TO EC2 in VPC 3 ($ec2_3_private_ip)"
ssh -i ./aws-workout-key-pair.pem -o ConnectTimeout=10 -J ec2-user@"$ec2_3_public_ip" ec2-user@"$ec2_2_private_ip" ping -c 2 "$ec2_3_private_ip"

echo "✅ Test Connection from EC2 in VPC 2 ($ec2_2_private_ip using forwarding agent) TO EC2 in VPC 1 ($ec2_1_private_ip)"
ssh -i ./aws-workout-key-pair.pem -o ConnectTimeout=10 -J ec2-user@"$ec2_3_public_ip" ec2-user@"$ec2_2_private_ip" ping -c 2 "$ec2_1_private_ip"

echo "❌ VPC Peering is not transitive ($ec2_3_public_ip) CAN'T REACH EC2 in VPC 1 ($ec2_1_private_ip)"
ssh -i ./aws-workout-key-pair.pem -o ConnectTimeout=10 ec2-user@"$ec2_3_public_ip" ping -c 2 "$ec2_1_private_ip"
