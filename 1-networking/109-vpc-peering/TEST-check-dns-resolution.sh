...
ssh -i ./aws-workout-key-pair.pem -o ConnectTimeout=10 -o StrictHostKeyChecking=no ec2-user@"$ec2_3_public_ip" nslookup "$ec2_2_private_dns"
...
ssh -i ./aws-workout-key-pair.pem -o ConnectTimeout=10 -o StrictHostKeyChecking=no ec2-user@"$ec2_3_public_ip" nslookup "$ec2_2_public_dns"

echo "✅ DNS Resolution from public DNS ($ec2_1_public_dns) resolves to private IP ($ec2_1_private_ip) - 👉 if VPC peering is setup this way -e.g. peering from VPC1 to VPC2"
ssh -i ./aws-workout-key-pair.pem -o ConnectTimeout=10 -o StrictHostKeyChecking=no -J ec2-user@"$ec2_3_public_ip" ec2-user@"$ec2_2_private_ip" nslookup "$ec2_1_public_dns"
ssh -i ./aws-workout-key-pair.pem -o ConnectTimeout=10 -o StrictHostKeyChecking=no -J ec2-user@"$ec2_3_public_ip" ec2-user@"$ec2_2_private_ip" nslookup "$ec2_1_private_dns"
