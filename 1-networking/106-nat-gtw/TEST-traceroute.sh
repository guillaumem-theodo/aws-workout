...
ssh -i ./aws-workout-key-pair.pem -o ConnectTimeout=10 -o StrictHostKeyChecking=no ec2-user@"$ec2_1_public_ip" traceroute -m 2 pokeapi.co

echo "✅ Traceroute internet through NAT Gateway (from Private EC2) - first HOP is 👉 NAT gateway private ip:$nat_gtw_private_ip"
ssh -i ./aws-workout-key-pair.pem -o ConnectTimeout=10 -o StrictHostKeyChecking=no -J ec2-user@"$ec2_1_public_ip" ec2-user@"$ec2_2_private_ip" traceroute -m 2 pokeapi.co
