...
ssh -i ./aws-workout-key-pair.pem -o ConnectTimeout=10 -o StrictHostKeyChecking=no ec2-user@"$ec2_1_public_ip" nslookup "$ec2_2_private_dns"

echo "✅ Public DNS name '$ec2_1_public_dns' can be used from WITHIN the VPC - should resolve to private IP"
ssh -i ./aws-workout-key-pair.pem -o ConnectTimeout=10 -o StrictHostKeyChecking=no ec2-user@"$ec2_1_public_ip" nslookup "$ec2_1_public_dns"
