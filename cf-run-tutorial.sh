...
if [ -f "$1"/dep.txt ]; then
  ...
  do
   ...
   # Check if the cloudformation stack of the dependency has been applied successfully
    found=$(aws cloudformation list-stacks --region eu-west-1 --profile aws-workout --query "StackSummaries[?StackName == 'stack-$line'].StackName" --stack-status-filter CREATE_COMPLETE --output text)
    if [ -z "$found" ]; then
